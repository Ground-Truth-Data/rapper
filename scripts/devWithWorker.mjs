/**
 * `npm run dev` = vite + the mounted child's local tile worker. Skips the
 * worker, never fails the dev server, when no child ships
 * workers/worker-local-dev or :8787 already answers.
 */
...
const children = [];
// ⚠️ detached + group-kill: killing the `npm run` intermediary alone ORPHANS
// the real vite/wrangler beneath it.
const start = (cmd, args, opts, name) => {
	const p = spawn(cmd, args, { stdio: "inherit", detached: true, ...opts });
	p.on("exit", (code) => {
		// The worker dying just logs — the app still works against cloud tiers.
